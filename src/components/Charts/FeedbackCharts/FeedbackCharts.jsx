...
const FeedbackCharts = ({ data }) => {
  // تابع برای رندر نمودار دایره‌ای ساده
  ...
  // تابع برای رندر توزیع امتیازات - طراحی جدید
const renderRatingDistribution = (data) => {
        ...
        {data.map((item, index) => (
          <div key={index} className="rating-item">
            <div className="rating-stars-container">
              ...
              <span className="rating-stars-label">{item.rating} Star{item.rating !== 1 ? 's' : ''}</span>
            </div>
  ...
  return (
    <div className="feedback-charts">
      ...
      <div className="feedback-charts-subtitle">Visual insights from user feedback data</div>
      ...
      <div className="charts-summary">
        <div className="summary-item">
          ...
          <span className="summary-value">
            {data.timeline.reduce((sum, item) => sum + item.comments, 0).toLocaleString()}
          </span>
        </div>
        <div className="summary-item">
        ...
        <div className="summary-item">
          ...
          <span className="summary-value">
            {(
              data.ratingDistribution.reduce((sum, item) => sum + (item.rating * item.percentage), 0) / 100
            ).toFixed(1)}
          </span>
        </div>
      </div>
